...
Notification emails can now display **two** logos (for example the CP-KKU badge and the Khon Kaen University wordmark). Choose whichever option works for your deployment:
...
For backwards compatibility you can still use the single-value variables `EMAIL_LOGO_URL` or `EMAIL_LOGO_PATH`. If nothing is configured the system falls back to two default hosted images so the header always renders with branding.
