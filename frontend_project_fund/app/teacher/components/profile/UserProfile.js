  ...
  const loadProfileData = async () => {
    try {
      ...
      const totalRequested =
        myApps.total_requested || myApps.totalRequested || 0;
      const totalApproved = myApps.total_approved || myApps.totalApproved || 0;
      const submissionCount = myApps.total || myApps.Total || 0;
      const usedBudget = budget.used_budget || budget.UsedBudget || 0;
      const remainingBudget =
        budget.remaining_budget || budget.RemainingBudget || 0;
      ...
      setTeacherData({
        ...
        stats: {
          totalApplications: submissionCount,
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">ขอทั้งหมด</p>
              <p className="text-xl font-bold text-gray-900">
                {formatCurrency(teacherData.stats.totalRequested)}
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">อนุมัติทั้งหมด</p>
              <p className="text-xl font-bold text-gray-900">
                {formatCurrency(teacherData.stats.totalApproved)}
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">คงเหลือ</p>
              <p className="text-xl font-bold text-gray-900">
                {formatCurrency(teacherData.stats.remainingBudget)}
              </p>
            </div>
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="text-sm text-gray-600">จำนวนคำร้อง</p>
              <p className="text-xl font-bold text-gray-900">
                {teacherData.stats.submissionCount.toLocaleString()}
              </p>
            </div>
          </div>
        </section>
