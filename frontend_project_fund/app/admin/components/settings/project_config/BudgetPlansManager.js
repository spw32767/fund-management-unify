...
import { Wallet, PlusCircle, RefreshCcw, Save, Pencil, GripVertical } from "lucide-react";
...
export default function BudgetPlansManager() {
  ...
  const loadPlans = async () => {
    setLoading(true);
    try {
      const list = await adminAPI.getProjectBudgetPlans();
      ...
      console.error(error);
      Toast.fire({ icon: "error", title: "ไม่สามารถโหลดแผนงบประมาณได้" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const editingId = editingItem?.plan_id ?? null;
    const duplicate = items.some(
      (item) => item?.name_th?.trim().toLowerCase() === normalized && (item?.plan_id ?? null) !== editingId
    );
    ...
    try {
      setSaving(true);
      if (editingItem) {
        await adminAPI.updateProjectBudgetPlan(editingItem.plan_id, payload);
  ...
  const handleToggleActive = async (item) => {
    ...
    try {
      await adminAPI.updateProjectBudgetPlan(id, { is_active: next });
      setItems((prev) =>
        prev.map((entry) => ((entry.plan_id ?? entry.id) === id ? { ...entry, is_active: next } : entry))
      );
      Toast.fire({ icon: "success", title: next ? "เปิดใช้งานแผนงบประมาณแล้ว" : "ปิดใช้งานแผนงบประมาณแล้ว" });
    } catch (error) {
      console.error(error);
  ...
  const handleDragOver = (event, item) => {
    ...
    setItems((prev) => {
      const updated = [...prev];
      const fromIndex = updated.findIndex((entry) => (entry.plan_id ?? entry.id) === draggingId);
      const toIndex = updated.findIndex((entry) => (entry.plan_id ?? entry.id) === overId);
  ...
  const handlePersistOrder = async () => {
    ...
    try {
      setSavingOrder(true);
      const payload = items.map((item) => item.plan_id ?? item.id);
      await adminAPI.reorderProjectBudgetPlans(payload);
    ...
    return (
      <div className="space-y-3">
        {items.map((item, index) => {
          const itemId = item.plan_id ?? item.id ?? index;
              ...
              <div className="flex items-center gap-3 min-w-[220px]">
                ...
                <div>
                  <div className="text-sm font-semibold text-gray-900">
                    {item.name_th || "-"}
                    <span className="text-xs text-gray-400 ml-2">ID: {item.plan_id ?? "-"}</span>
                  </div>
  ...
  return (
    <>
      <SettingsSectionCard
        ...
        actions={
          <div className="flex items-center gap-3">
            ...
            >
              <PlusCircle size={16} /> เพิ่มแผนงบประมาณ
            </button>
          </div>
        }
      ...
      <SettingsModal
        ...
        title={editingItem ? "แก้ไขแผนงบประมาณ" : "เพิ่มแผนงบประมาณ"}
      ...
      >
        <form id="budget-plan-form" onSubmit={handleSubmit} className="space-y-4">
          <div>
