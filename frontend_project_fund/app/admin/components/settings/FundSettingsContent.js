...
export default function FundSettingsContent({ onNavigate }) {
  ...
  const loadYears = async ({ preserveSelection = false, systemYearOverride = null } = {}) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const systemYearValue =
        systemYearOverride ?? currentYearRes?.current_year ?? currentYearRes?.data?.current_year ?? null;
  ...
  const loadYearsRef = useRef(loadYears);

  useEffect(() => {
    loadYearsRef.current = loadYears;
  }, [loadYears]);

  useEffect(() => {
    const triggerReload = (payload) => {
      const detail = payload?.detail ?? payload ?? null;
      const currentYear =
        detail?.currentYear ?? detail?.current_year ?? detail?.year ?? detail ?? null;

      loadYearsRef.current?.({ preserveSelection: false, systemYearOverride: currentYear });
    };

    const handleCurrentYearEvent = (event) => {
      triggerReload(event);
    };

    const handleStorageEvent = (event) => {
      if (event.key !== "system-current-year-updated" || !event.newValue) return;
      try {
        const parsed = JSON.parse(event.newValue);
        triggerReload({ detail: parsed });
      } catch (error) {
        console.warn("ไม่สามารถอ่านข้อมูลปีงบประมาณจาก localStorage", error);
      }
    };

    window.addEventListener("system-current-year-updated", handleCurrentYearEvent);
    window.addEventListener("storage", handleStorageEvent);

    return () => {
      window.removeEventListener("system-current-year-updated", handleCurrentYearEvent);
