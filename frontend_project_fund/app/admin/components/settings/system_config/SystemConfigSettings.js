...
export default function SystemConfigSettings() {
  ...
  async function loadConfig() {
    ...
    setForm((f) => ({
      ...
      max_submissions_per_year:
        normalized.max_submissions_per_year != null ? String(normalized.max_submissions_per_year) : "",
  ...
  const handleSave = async () => {
    ...
    const maxSubmissionsRaw = String(form.max_submissions_per_year || "").trim();
    const maxSubmissionsValue = maxSubmissionsRaw ? Number(maxSubmissionsRaw) : null;
    if (maxSubmissionsRaw && (!Number.isFinite(maxSubmissionsValue) || maxSubmissionsValue < 1)) {
      toast("warning", "กรุณาระบุจำนวนครั้งยื่นทุนสูงสุดต่อปีเป็นตัวเลขตั้งแต่ 1 ขึ้นไป");
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...
        max_submissions_per_year: maxSubmissionsValue,
      };
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-700">จำนวนครั้งยื่นทุนสูงสุดต่อปี</label>
                  <input
                    type="number"
                    min={1}
                    ...
                    placeholder="เช่น 5"
                  />
                  <p className="text-xs text-gray-500">กำหนดจำนวนครั้งรวมที่ยื่นทุนได้ต่อปีงบประมาณ</p>
                </div>
