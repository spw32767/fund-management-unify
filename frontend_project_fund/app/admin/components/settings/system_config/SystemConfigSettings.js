  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent("system-current-year-updated", { detail }));
        localStorage.setItem("system-current-year-updated", JSON.stringify(detail));
      } catch (notifyError) {
        console.warn("ไม่สามารถกระจายการอัปเดตปีงบประมาณได้", notifyError);
      }
    } catch (e) {
