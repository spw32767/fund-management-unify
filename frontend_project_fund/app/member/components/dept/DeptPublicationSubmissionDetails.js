...
export default function DeptPublicationSubmissionDetails({ submissionId, onBack }) {
  ...
  const approve = async (payload) => {
    const commentParts = [];
    if (payload?.announce_reference_number) {
      commentParts.push(`เลขอ้างอิงประกาศ: ${payload.announce_reference_number}`);
    }
    const amountsSummary = [
      typeof payload?.reward_approve_amount === 'number'
        ? `เงินรางวัล: ฿${formatCurrency(payload.reward_approve_amount)}`
        : null,
      typeof payload?.revision_fee_approve_amount === 'number'
        ? `ค่าปรับปรุง: ฿${formatCurrency(payload.revision_fee_approve_amount)}`
        : null,
      typeof payload?.publication_fee_approve_amount === 'number'
        ? `ค่าธรรมเนียมตีพิมพ์: ฿${formatCurrency(payload.publication_fee_approve_amount)}`
        : null,
      typeof payload?.total_approve_amount === 'number'
        ? `รวมอนุมัติ: ฿${formatCurrency(payload.total_approve_amount)}`
        : null,
    ].filter(Boolean);

    if (amountsSummary.length) {
      commentParts.push(amountsSummary.join(' | '));
    }
    ...
    if (commentText) {
      requestPayload.comment = commentText;
    }

    await deptHeadAPI.submitDecision(submission.submission_id, requestPayload);
  ...
  const reject = async (reason) => {
    ...
    if (trimmed) {
      requestPayload.comment = trimmed;
    }
